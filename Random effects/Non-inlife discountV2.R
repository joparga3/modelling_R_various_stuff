...
#d = subset(d, Pol_Dt_Renew >= '2017-08-01')
...
# Response
# ----------------------------------------------------
d = subset(d, Pol_Sale_Type == 'RD' | Pol_Sale_Type =='RR')
...
f = ~ 
      ...
      #I(pmin(log(Prem_Gross / Prem_Gross_PY),0.7)) +
      I(pmax(log(Prem_Gross / Prem_Gross_PY),0.4)) +
      
      # GROSS PREMIUM / NB PRICE GROSS
      I(pmin(log(Prem_Gross / NBPrice_Gross),0.7)) +
      I(pmin(log(Prem_Gross / NBPrice_Gross),0.2)) +

      # NB PRICE GROSS / MARKET PRICE
      #I(pmin(log(NBPrice_Gross / MarketPrice_Predicted),-0.5)) +
      I(pmin(log(NBPrice_Gross / MarketPrice_Predicted),0)) +
      I(pmax(log(NBPrice_Gross / MarketPrice_Predicted),0)) +
      I(pmax(log(NBPrice_Gross / MarketPrice_Predicted),0.5)) +
      
      # PREM GROSS THIS YEAR
      #I(pmin(log(Prem_Gross),5.2)) +
      I(pmin(log(Prem_Gross),5.9)) +
      I((pmax(log(Prem_Gross),5.5)^2)/10) +
  
      # MAX ALLOWED DISCOUNT
      I(Max_Allowed_Discount/10) +
      I(pmax(Max_Allowed_Discount,50)/10) +
  
      # MAX ALLOWED DISCOUNT WITH GROSS YOY
      I((pmax(Max_Allowed_Discount,124)/10)*(log(Prem_Gross / Prem_Gross_PY))) +
  
      # TENURE WITH NB/MARKET
      I(pmin(Pol_Tenure,2)*(pmax(log(NBPrice_Gross / MarketPrice_Predicted),0.3))) +
  
      # TENURE WITH GROSS YOY
      I(pmax(Pol_Tenure,2)*(log(Prem_Gross / Prem_Gross_PY))) + 
      I(pmin(Pol_Tenure,2)*(log(Prem_Gross / Prem_Gross_PY))) + 
...
re1_m = as.matrix(ranef(b1)$re1[match(ifelse(b$Pol_Dt_Renew > '2017-09-20',1,0), rownames(as.matrix(ranef(b1)$re1))),])
...
base = as.vector(1/(1+exp(-(re1_m
                            #+ re2_m
                            #+ re3_m
                            + (fixef(b1)[1]) + em %*% as.matrix(fixef(b1)[2:length(fixef(b1))])))))
...
minus5 = as.vector(1/(1+exp(-(re1_m
                              #+ re2_m
                              #+ re3_m
                              + (fixef(b1)[1]) + em %*% as.matrix(fixef(b1)[2:length(fixef(b1))])))))
...
ElasPlt(actual=br,base=base,plus=plus5,minus=minus5,
        factor=factor,
        ylab="Discount %",xlab="Gross Premium / NB Gross Premium",ylim=c(0,1),
        title="Impact of \U00b1 5% Price Change on No-Inlife Discount")
...
function_actuals_v_predicted_elasticity = function(act_data){
  ...
  plot(row.names(elasticity),elasticity$Actual.Dec., type="l", ylim = c(-10,5),
       ...
       main="Elasticity AvP - Non-inlife discount",col='Dark red',lty = 2, lwd=1.5,cex.main=1,cex.lab=1)
